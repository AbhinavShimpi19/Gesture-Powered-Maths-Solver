...
class Plotter:
    ...
    def plot_equation(self):
        ...
        count = 0
        m, c = None, None  # Initialize slope and intercept
            ...
            if len(landmark_list) != 0:
                ...
                if len(fingers) == 5:
                    if fingers[1] == 1 and fingers[2] == 1 and fingers[0] == 0 and fingers[3] == 0 and fingers[4] == 0:
                        cv2.rectangle(img, (x1 - 25, y1 - 25), (x1 + 25, y1 + 25), (0, 0, 255), cv2.FILLED)
                        if xp == 0 and yp == 0:
                            xp, yp = x1, y1
                        cv2.line(blkboard, (xp, yp), (x1, y1), (0, 0, 0), self.eraser_thick)
                        xp, yp = x1, y1  # Update to current finger position

                    elif fingers[1] == 1 and all(f == 0 for f in fingers[2:]):
                        cv2.circle(img, (x1, y1), 15, (255, 0, 255), cv2.FILLED)
                        if xp == 0 and yp == 0:  # Initial case
                            xp, yp = x1, y1
                        cv2.line(blkboard, (xp, yp), (x1, y1), (0, 255, 0), self.brush_thick)
                        xp, yp = x1, y1  # Update position

                    elif fingers[0] == 1 and all(f == 0 for f in fingers[1:]):
                        xp, yp = 0, 0  # Reset position
                        print("Finalize the equation")

                        # Process drawn content for prediction
                            ...
                            for cnt in blackboard_cnts:
                                if cv2.contourArea(cnt) > 500:
                                    ...
                                    if abs(x - 910) <= 50:
                                        ...
                                        pred_probab, m = self.plot_model.predict(digit)
                                    if abs(x - 1150) <= 50:
                                        digit = blackboard_gray[y:y + h, x:x + w]
                                        pred_probab, c = self.plot_model.predict(digit)

                            # Plotting the equation
                            if m is not None and c is not None:  # Ensure both values are valid
                                plt.clf()  # Clear previous plot
                                plt.grid(color='green', linestyle='--', linewidth=0.5)
                                plt.axvline(x=0, c="black", label="x=0")
                                plt.axhline(y=0, c="black", label="y=0")
                                plt.plot(x_plot, m * x_plot + c, linestyle='solid', color="blue")
                                plt.title(f"Socratic Plot for y = {m}x + {c}")
                                plt.xlabel("X")
                                plt.ylabel("Y")
                                fig.canvas.draw()

            # Combine the blackboard and the current frame
